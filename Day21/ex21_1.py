...
allergen_ingredients = {ingredient for ingredients in allergens.values() for ingredient in ingredients}

# On compte le nombre d'occurence des ingrédients qui ne sont pas allergènes
total = 0
for food_ingredients, _ in foods:
    for food_ingredient in food_ingredients:
        if food_ingredient not in allergen_ingredients:
            total += 1
